import React from 'react';
import { Link } from 'react-router-dom';
import { AiFillDashboard } from 'react-icons/ai'
import { MdAddCircle, MdUpdate, MdViewList } from 'react-icons/md';
import { BiLogOutCircle } from 'react-icons/bi'
import { useDispatch, useSelector } from 'react-redux';
import { logout } from '../../store/actions/auth-actions';


const TheSidebar = () => {
    const dispatch = useDispatch();
    const token = useSelector((state) => state.auth.token);


    const logoutUser = async () => {
        await dispatch(logout(token));
    };


    return (
      <div className="flex flex-col h-full px-6 bg-green-500">
        <div className="mb-12 mt-8 flex flex-col justify-center items-center">
          <span className="block text-6xl text-white">
            <AiFillDashboard />
          </span>
          <h1 className="uppercase text-3xl tracking-wider font-extrabold text-center">
            <span className="text-white">DASH</span>
            <span className="text-green-200">BOARD</span>
          </h1>
        </div>
        <div className="flex flex-col text-gray-200 font-semibold tracking-wider text-lg space-y-3">
          <Link
            to="/admin/dashboard"
            className="border-b-2 pb-3 border-gray-500"
          >
            <span className="inline-flex mr-3 text-primary">
              <AiFillDashboard />{" "}
            </span>
            Dashboard
          </Link>
          <Link to="/products" className="border-b-2 pb-3 border-gray-500">
            <span className="inline-flex mr-3 text-primary">
              <MdViewList />{" "}
            </span>
            Liste Produits
          </Link>
          <Link
            to="/admin/dashboard/addproduct"
            className="border-b-2 pb-3 border-gray-500"
          >
            <span className="inline-flex mr-3 text-primary">
              <MdAddCircle />{" "}
            </span>
            Ajouter un Produit
          </Link>
          <Link
            to="/admin/dashboard/updateproducts"
            className="border-b-2 pb-3 border-gray-500"
          >
            <span className="inline-flex mr-3 text-primary">
              <MdUpdate />{" "}
            </span>
            Update Produit
          </Link>
          <Link to="/orders" className="border-b-2 pb-3 border-gray-500">
            <span className="inline-flex mr-3 text-primary">
              <MdViewList />{" "}
            </span>
            Commandes
          </Link>
        </div>
        <div className="mt-auto mb-8">
          <button
            className="px-4 py-2 bg-white rounded-md flex items-center shadow-lg"
            onClick={logoutUser}
          >
            <span className="inline-flex mr-3 font-bold">
              <BiLogOutCircle />
            </span>
            Deconnexion
          </button>
        </div>
        {/* <ul>
                <li>
                    <Link to='/admin/dashboard/main'>Test</Link>
                </li>
            </ul> */}
      </div>
    );
};


export default TheSidebar;